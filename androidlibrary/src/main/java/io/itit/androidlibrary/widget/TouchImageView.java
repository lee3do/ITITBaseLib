...
public class TouchImageView extends ImageView {
                ...
                switch (event.getAction() & MotionEvent.ACTION_MASK) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (mode == DRAG) {
                            ...
                            velocity = (float) distanceBetween(curr, last) / (dragTime -
                                    lastDragTime) * FRICTION;
                            ...
                            if (10 > Math.abs(oldDist - newDist) || Math.abs(oldDist - newDist) >
                                    50)
                                break;
                            ...
                            if (origWidth * saveScale <= width || origHeight * saveScale <=
                                ...
                                if (mScaleFactor < 1) {
                                    ...
                                    else if (matrixX > 0) matrix.postTranslate(-matrixX, 0);
                                    if (matrixY < -bottom)
                                        matrix.postTranslate(0, -(matrixY + bottom));
                                    else if (matrixY > 0) matrix.postTranslate(0, -matrixY);
                                }
                            }
                            checkSiding();
                        }
    ...
    private void checkAndSetTranslate(float deltaX, float deltaY) {
        ...
        if (scaleWidth < width) {
            deltaX = 0;
            if (matrixY + deltaY > 0) deltaY = -matrixY;
            else if (matrixY + deltaY < -bottom) deltaY = -(matrixY + bottom);
        } else if (scaleHeight < height) {
            deltaY = 0;
            if (matrixX + deltaX > 0) deltaX = -matrixX;
            else if (matrixX + deltaX < -right) deltaX = -(matrixX + right);
        } else {
            if (matrixX + deltaX > 0) deltaX = -matrixX;
            else if (matrixX + deltaX < -right) deltaX = -(matrixX + right);
    ...
    private void checkSiding() {
        ...
        // matrixX));
        if ((scaleWidth >= width && (matrixX + scaleWidth - width) < 10) || (scaleWidth <= width
                && -matrixX + scaleWidth <= width))
            onRightSide = true;
